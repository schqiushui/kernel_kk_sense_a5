				   ...
				   const char *buf, size_t count)
{
	...
	if (Mode == 0) {
		lp5521_led_off(client);
	} else if (Mode == 1) {	/* === set red, green, blue direct control === */
		mutex_lock(&led_mutex);
		...
		udelay(500);
		mutex_unlock(&led_mutex);
	} else if (Mode == 2) { /* === set short blink === */
		lp5521_color_blink(client, Red, Green, Blue);
	} else if (Mode == 3) { /* === set delayed short blink === */
	...
	} else if (Mode == 4 && Red && !Green && !Blue) { /* === set red long blink === */
		lp5521_red_long_blink(client);
	} else if (Mode ==5 && Red && Green && !Blue) { /* === set red green blink === */
